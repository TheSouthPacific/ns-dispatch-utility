...
class TestReplaceNameWithPersonnelInfo():
    def test_with_existing_info(self):
        personnel_info = {'Frodo': {'name': 'Frodo', 'nation': 'Frodonia', 'discord_handle': 'Frodo#1234'},
                          'Gandalf': {'name': 'Frodo', 'nation': 'Gandalf Republic', 'discord_handle': 'Gandalf#4321'}}

        r = file_templatevarloader.replace_name_with_personnel_info('Frodo', personnel_info)

        assert r == {'name': 'Frodo', 'nation': 'Frodonia', 'discord_handle': 'Frodo#1234'}
    ...
    def test_with_non_existing_info(self):
        ...
        with pytest.raises(exceptions.LoaderConfigError):
            file_templatevarloader.replace_name_with_personnel_info('Random', personnel_info)


class TestReplaceNameListWithPersonnelInfo():
    def test_with_existing_info(self):
        personnel_info = {'Frodo': {'name': 'Frodo', 'nation': 'Frodonia', 'discord_handle': 'Frodo#1234'},
                          'Gandalf': {'name': 'Gandalf', 'nation': 'Gandalf Republic', 'discord_handle': 'Gandalf#4321'}}

        r = file_templatevarloader.replace_name_list_with_personnel_info(['Frodo', 'Gandalf'], personnel_info)

        assert r == [{'name': 'Frodo', 'nation': 'Frodonia', 'discord_handle': 'Frodo#1234'},
                     {'name': 'Gandalf', 'nation': 'Gandalf Republic', 'discord_handle': 'Gandalf#4321'}]
    ...
    def test_with_non_existing_info(self):
        ...
        with pytest.raises(exceptions.LoaderConfigError):
            file_templatevarloader.replace_name_list_with_personnel_info(['Frodo', 'Random'], personnel_info)


class TestMergePersonnelInfoGroups():
    def test_with_existent_groups(self):
        template_vars = {'info1': {'Frodo': {'nation': 'Frodonia', 'discord_handle': 'Frodo#1234'},
        ...
        r = file_templatevarloader.merge_personnel_info_groups(template_vars, ['info1', 'info2'])

        expected = {'Frodo': {'name': 'Frodo', 'nation': 'Frodonia', 'discord_handle': 'Frodo#1234'},
                    'Gandalf': {'name': 'Gandalf', 'nation': 'Gandalf Republic', 'discord_handle': 'Gandalf#4321'},
                    'Theoden': {'name': 'Theoden', 'nation': 'Theoden Federation', 'discord_handle': 'Theoden#0974'}}
        assert r == expected
    ...
    def test_with_non_existent_groups(self):
        ...
        with pytest.raises(exceptions.LoaderConfigError):
            file_templatevarloader.merge_personnel_info_groups(template_vars, ['info1', 'random'])


class TestAddPersonnelInfo():
    def test_with_existing_groups(self):
        personnel = {'personnel1': {'position1': 'Frodo', 'position2': ['Gandalf', 'Sauron']},
        ...
        expected = {'personnel1': {'position1': {'name': 'Frodo', 'nation': 'Frodonia', 'discord_handle': 'Frodo#1234'},
                                   'position2': [{'name': 'Gandalf', 'nation': 'Gandalf Republic', 'discord_handle': 'Gandalf#4321'},
                                                 {'name': 'Sauron', 'nation': 'Sauron', 'discord_handle': 'Sauron#5050'}]},
                    'personnel2': {'position1': {'name': 'Theoden', 'nation': 'Theoden Federation', 'discord_handle': 'Theoden#0974'}},
                    'foo1': 'bar1',
...
class TestFileVarLoader():
    def test_integration(self, toml_files):
        vars_1 = {'personnel1': {'position1': 'Frodo', 'position2': ['Gandalf', 'Sauron']},
                  'personnel2': {'position1': 'Theoden'}}
